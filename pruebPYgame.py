...
animaciones = []
for i in range(3):
    img = pygame.image.load(f"EDATACHI/assets/images/characters/player/player {i} w.png")
...
for x in range(constantes.TILE_TYPES):
    tile_image = pygame.image.load(f"EDATACHI/assets/images/tiles/tile_ ({x+1}).png")
...
#mundo bello
with open("EDATACHI/mundo_archivo/mapalol.csv",newline='') as csvfile:
...
player_image = pygame.image.load("EDATACHI/assets/images/characters/player/player 2 w.png")
...
def ponerCasa(archivo, click_pos, nuevo_valor):
    ...
    # Leer todo el CSV
    with open("EDATACHI/mundo_archivo/mapalol.csv", newline='', encoding='utf-8') as f:
        lector = csv.reader(f)
        datos = list(lector)
    ...
    with open("EDATACHI/mundo_archivo/mapalol.csv", 'w', newline='', encoding='utf-8') as f:
...
while run:
    ...
    for event in  pygame.event.get():
        ...
        if event.type == pygame.MOUSEBUTTONDOWN:
            if event.button == 1:
                ponerCasa("EDATACHI/mundo_archivo/mapalol.csv",event.pos,195)
